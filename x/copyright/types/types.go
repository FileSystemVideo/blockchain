...
//trade base
func (this *TradeBase) UpdateTradeBase(tradeType core.TranserType, contractAddr string) {
...
func (t *TransferData) UpdateFee(prop *TxPropValue) error {
	for _, event := range prop.Events {
		if event.Type == EventTypeTransferFee {
			for _, attr := range event.Attributes {
				if string(attr.Key) == AttributeKeyTransferFee {
					coins, err := sdk.ParseCoinsNormalized(string(attr.Value))
					realCoins := MustLedgerCoins2RealCoins(coins)
					if err != nil {
						return err
					}
					for _, coin := range realCoins {
						if coin.Denom == core.MainToken {
							amount, err := decimal.NewFromString(coin.Amount)
							if err != nil {
								return err
							}
							t.Fee = NewLedgerFee(amount)
						}
					}
				}
			}
...
func (t *TransferData) UpdateBalance(prop *TxPropValue, fromAddr, toAddr string) error {
	for _, event := range prop.Events {
		if event.Type == types.EventTypeTransfer {
			for _, attr := range event.Attributes {
				if string(attr.Key) == types.AttributeKeySender {
					from := string(attr.Value)
					if from != fromAddr {
						break
					}
				}
				if string(attr.Key) == types.AttributeKeyRecipient {
					to := string(attr.Value)
					
					if to != toAddr || to == core.ContractAddressFee.String() {
						break
					}
				}
				if string(attr.Key) == types.AttributeKeyRecipientBalance {
					coins, err := sdk.ParseCoinsNormalized(string(attr.Value))
					realCoins := MustLedgerCoins2RealCoins(coins)
					if err != nil {
						return err
					...
					for _, coin := range realCoins {
						if coin.Denom == core.MainToken {
							t.ToFsvBalance = coin.Amount
						} else {
							t.ToTipBalance = coin.Amount
						}
					}
				}
				if string(attr.Key) == types.AttributeKeySenderBalance {
					coins, err := sdk.ParseCoinsNormalized(string(attr.Value))
					realCoins := MustLedgerCoins2RealCoins(coins)
					if err != nil {
						return err
					...
					for _, coin := range realCoins {
						if coin.Denom == core.MainToken {
							t.FromFsvBalance = coin.Amount
						} else {
							t.FromTipBalance = coin.Amount
						}
					}
				}
			}
...
type IbcTransferData struct {
	...
	SendAddress   string   `json:"send_address"`   
	ToAddress     string   `json:"to_address"`     
	Token         RealCoin `json:"token"`          
	SourcePort    string   `json:"source_port"`    
	SourceChannel string   `json:"source_channel"` //ID
}
