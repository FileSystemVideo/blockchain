...
//
func (k Keeper) AuthenticationValidator(ctx sdk.Context, cancelBech32ConsAddress string, consAddress sdk.ConsAddress) error {
	store := ctx.KVStore(k.storeKey)
	if cancelBech32ConsAddress != "" {
		store.Delete([]byte(types.ValidNodePubKey + cancelBech32ConsAddress))
	}
	//
	store.Set([]byte(types.ValidNodePubKey+consAddress.String()), []byte(ctx.BlockTime().String()))
	return nil
}

//
func (k Keeper) GetAuthenticationValidator(ctx sdk.Context, consAddress sdk.ConsAddress) bool {
	store := ctx.KVStore(k.storeKey)
	validatorBindInfor := store.Get([]byte(types.ValidNodePubKey + consAddress.String()))
...
func (k Keeper) withdrawDelegationRewards(ctx sdk.Context, val stakingtypes.ValidatorI, del stakingtypes.DelegationI) (sdk.Coins, error) {
	...
	if ctx.BlockHeight() < config.Upgrade20220119Height {
