...
func NewCoinsFromDecCoins(dcoins ...DecCoin) Coins {
	coins := make(Coins, len(dcoins))
	newDecCoins := NewDecCoins(dcoins...)
	for i, coin := range newDecCoins {
		coins[i] = NewCoinFromDecCoin(coin)
	}

	return coins
}

func NewCoinFromDecCoin(dcoin DecCoin) Coin {
	return Coin{
		Denom:  dcoin.Denom,
		Amount: dcoin.Amount.TruncateInt(),
	}
}
