...
`,
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			ctrlCtx := GetCtrlContextFromCmd(cmd)
			...
			err = startInProcess(serverCtx, clientCtx, appCreator, ctrlCtx)
	...
	cmd.Flags().String(flagLogFile, "", "log file path")
...
var NodeConsAddress tmTypes.Address
var NodePrivateKey crypto.PrivKey

func startInProcess(ctx *Context, clientCtx client.Context, appCreator types.AppCreator, ctrlCtx context.Context) error {
	cfg := ctx.Config
	home := cfg.RootDir
	...
	if gRPCOnly {
		...
		privvval := pvm.LoadOrGenFilePV(cfg.PrivValidatorKeyFile(), cfg.PrivValidatorStateFile())

		NodeConsAddress = privvval.GetAddress()
		NodePrivateKey = privvval.Key.PrivKey
