...
func NewQuerier(k Keeper, legacyQuerierCdc *codec.LegacyAmino) sdk.Querier {
	return func(ctx sdk.Context, path []string, req abci.RequestQuery) ([]byte, error) {
		switch path[0] {
		case types.QueryValidatorsName:
			return queryValidatorsName(ctx, req, k, legacyQuerierCdc)
...
func queryValidatorsName(ctx sdk.Context, req abci.RequestQuery, k Keeper, legacyQuerierCdc *codec.LegacyAmino) ([]byte, error) {
	var params types.QueryValidatorsNameParams
	...
	responseResult := make(map[string]string)

	for _, val := range validators {
		for _, addr := range params.Addrs {
			if val.OperatorAddress == addr.String() {
				responseResult[addr.String()] = val.Description.Moniker
			}
		}
	}
	res, err := codec.MarshalJSONIndent(legacyQuerierCdc, responseResult)
	if err != nil {
		return nil, sdkerrors.Wrap(sdkerrors.ErrJSONMarshal, err.Error())
	}
