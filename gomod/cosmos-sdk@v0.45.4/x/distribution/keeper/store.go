...
func (k Keeper) SetFeePool(ctx sdk.Context, feePool types.FeePool) {
	...
	if store.Has(types.FeePoolKey) {
		oldFee := k.GetFeePool(ctx)
		//event
		if oldFee.CommunityPool.String() != feePool.CommunityPool.String() {
			ctx.EventManager().EmitEvent(
				sdk.NewEvent(
					"feePoolEvent",
					sdk.NewAttribute("feePool", feePool.CommunityPool.String()),
				),
			)
		}
	}
