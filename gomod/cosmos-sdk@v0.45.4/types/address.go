...
const (
	...
	BlackHoleAddress = "00000000000000000000" 

	// Bech32MainPrefix defines the main SDK Bech32 prefix of an account's address
	Bech32MainPrefix = "fsv"
...
func AccAddressFromBech32(address string) (addr AccAddress, err error) {
	...
	bech32PrefixEthAccAddr := GetConfig().GetBech32AccountEthAddrPrefix()

	bz, err := GetFromBech32(address, bech32PrefixAccAddr, bech32PrefixEthAccAddr)
	if err != nil {
		return nil, err
	}
...
func (aa AccAddress) StringDex() string {
	if aa.Empty() {
		return ""
	}
	bech32Addr, err := bech32.ConvertAndEncode(GetConfig().GetBech32AccountEthAddrPrefix(), aa)
	if err != nil {
		panic(err)
	}
	return bech32Addr
...
func GetFromBech32(bech32str string, prefixs ...string) ([]byte, error) {
	if len(bech32str) == 0 {
	...
	return nil, fmt.Errorf("invalid Bech32 prefix")
}
...
func cacheBech32Addr(prefix string, addr []byte, cache *simplelru.LRU, cacheKey string) string {
	...
	if bytes.Equal(addr, []byte(BlackHoleAddress)) {
		return prefix + BlackHoleAddress + BlackHoleAddress[1:]
	}
