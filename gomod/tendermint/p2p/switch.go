...
type Switch struct {
	...
	//availabilityProbe map[string]time.Time
	availabilityProbe sync.Map
}
...
//
func (sw *Switch) AvailabilityProbe(addr *NetAddress) bool {
	nowTime := time.Now()
	//
	if doneTime, ok := sw.availabilityProbe.Load(addr.String()); ok {
		doTime := doneTime.(time.Time)
		if doTime.After(nowTime) { //
			return false
		}
	}

	//
	nextTime := nowTime.Add(availabilityProbeInterval)
	sw.availabilityProbe.Store(addr.String(), nextTime)

	//
	if sw.dialing.Has(string(addr.ID)) {
		return false
	}

	//
	if sw.peers.Has(addr.ID) || (!sw.config.AllowDuplicateIP && sw.peers.HasIP(addr.IP)) {
		sw.Logger.Debug("address only exist!")
		return true
	}

	//
	peer, err := sw.transport.Dial(*addr, peerConfig{
		chDescs:      sw.chDescs,
		onPeerError:  sw.StopPeerForError,
		isPersistent: sw.IsPeerPersistent,
		reactorsByCh: sw.reactorsByCh,
	...
	sw.Logger.Debug("AvailabilityProbe ", " ip:", addr.IP, " port:", addr.Port, " nowTime:", nowTime, " next:", nextTime, " error:", err)
	...
	sw.Logger.Debug("add Peer success.!")
	return true
}
