...
func (k Keeper) WithdrawValidatorCommission(ctx sdk.Context, valAddr sdk.ValAddress) (sdk.Coins, error) {
	...
	if ctx.BlockHeight() < config.Upgrade20220119Height {
		...
		k.SetValidatorAccumulatedCommission(ctx, valAddr, types.ValidatorAccumulatedCommission{Commission: sdk.NewDecCoins(sdk.NewDecCoin(sdk.DefaultBondDenom, sdk.ZeroInt()))}) // leave remainder to withdraw later
	}
