...
func InterceptConfigsPreRunHandler(cmd *cobra.Command) error {
	...
	if logFile != "" {
		logWriter, err = os.OpenFile(logFile, os.O_WRONLY|os.O_TRUNC|os.O_CREATE, os.ModePerm)
		if err != nil {
			return err
		}
	}
...
func GetCtrlContextFromCmd(cmd *cobra.Command) context.Context {
	if v := cmd.Context().Value("runctrl.context"); v != nil {
		runCtrlCtx := v.(context.Context)
		return runCtrlCtx
	}
	return nil
}
