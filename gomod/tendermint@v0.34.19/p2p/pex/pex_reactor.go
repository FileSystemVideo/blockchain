...
func (r *Reactor) GetAddrbook() AddrBook {
	return r.book
}
...
func (r *Reactor) AddPeer(p Peer) {
	if p.IsOutbound() {
		...
		srcNetAddr := p2p.NewNetAddress(addr.ID, p.RemoteAddr())
		srcNetAddr.Port = addr.Port 
		r.CheckAddress2Save(srcNetAddr)
...
peer,
*/
func (r *Reactor) CheckAddress2Save(srcNetAddr *p2p.NetAddress) error {

	
	addr := r.book.GetAddress(srcNetAddr) //addr=""
	if addr == srcNetAddr.String() {      
		r.Logger.Debug("CheckAddress2Save() It's up to date", " addr", srcNetAddr)
		return errors.New("The address already exists " + srcNetAddr.String())
	}

	
	if r.Switch.AvailabilityProbe(srcNetAddr) {
		
		if addr != "" {
			r.book.RemoveAddress(srcNetAddr)
		}
		err := r.book.AddAddress(srcNetAddr, srcNetAddr)
		r.Logger.Info("CheckAddress2Save()->AddAddress ", " addr", srcNetAddr, " error:", err)
		return err
	} else {
		return errors.New("availability probe fail " + srcNetAddr.String())
	}
}
	...
	case *tmp2p.PexRequest:
		...
		if r.config.SeedMode && !src.IsOutbound() {
			...
			go func() {
				...
				srcNodeAddr, _ := src.NodeInfo().NetAddress()
				srcNetAddr := p2p.NewNetAddress(srcNodeAddr.ID, src.RemoteAddr())
				srcNetAddr.Port = srcNodeAddr.Port 
				r.CheckAddress2Save(srcNetAddr)
...
func (r *Reactor) ReceiveAddrs(addrs []*p2p.NetAddress, src Peer) error {
	...
	for _, netAddr := range addrs {
		// peer,
		err := r.CheckAddress2Save(netAddr)
