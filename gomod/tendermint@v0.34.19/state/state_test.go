...
func TestProposerPriorityDoesNotGetResetToZero(t *testing.T) {
	...
	wantVal1Prio -= avg.Int64() 

	// 4. Steps from IncrementProposerPriority
	wantVal1Prio += val1VotingPower 
	wantVal2Prio += val2VotingPower 
	...
	wantVal1Prio += updatedVal1.VotingPower 
...
func TestProposerPriorityProposerAlternates(t *testing.T) {
	...
	totalPower = val1VotingPower + val2VotingPower           
	...
	expectedVal1Prio := oldVal1.ProposerPriority - avg.Int64() 
	// 4. Increment
	...
	for i := 0; i < 1000; i++ {
		...
		if i%2 == 0 {
			...
			assert.Equal(t, expectedVal2Prio, updatedVal2.ProposerPriority) 
		} else {
